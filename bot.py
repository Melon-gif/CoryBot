...
TOKEN= os.getenv ('TELEGRAM_TOKEN', "8921288173:AAHt7QWIzfDwJoRAJu-SsGTTAl_DdYVBcpw")
...
def console_listener():
    print("📟 Консольное управление активно.")
    print(f"📤 Сообщения будут отправляться в чат: {TARGET_CHAT_ID}")
    print("Команды:\n  Кори текст — отправить\n  /exit — выйти")
    while True:
        try:
            cmd=input().strip()
            if not cmd:
            ...
            if cmd.lower()=="/exit":
                print("👋 Завершение...")
                os._exit(0)
            if cmd.lower().startswith("кори "):
                text=cmd[5:].strip()
                if not text:
                    print("❌ Текст не может быть пустым")
                    continue
                try:
                    bot.send_message(TARGET_CHAT_ID,f"Сообщение из консоли: {text}")
