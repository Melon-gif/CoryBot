...
if not DATABASE_URL:
    print("❌ DATABASE_URL не найден! Использую локальную SQLite...")
    # Если нет Supabase, используем SQLite как запасной вариант
    import sqlite3
...
FOLDERS = {
    "memes": os.path.join(BASE_DIR, "memes"),
    "sleep": os.path.join(BASE_DIR, "sleep"),
    "awake": os.path.join(BASE_DIR, "awake"),
    "hug": os.path.join(BASE_DIR, "hug"),
    "kiss": os.path.join(BASE_DIR, "kiss"),
    "pet": os.path.join(BASE_DIR, "pet"),
    "coin": os.path.join(BASE_DIR, "coin")
}

# Проверка папок при запуске
for name, path in FOLDERS.items():
    if not os.path.exists(path):
        os.makedirs(path, exist_ok=True)
        print(f"📁 Создана папка: {name}")
    else:
        print(f"✅ Папка существует: {name}")
...
def create_tables():
    ...
    tables = [
        ...
        # Лимит фида
        """
        CREATE TABLE IF NOT EXISTS feed_limit (
            id SERIAL PRIMARY KEY,
            global_limit INTEGER DEFAULT 0,
